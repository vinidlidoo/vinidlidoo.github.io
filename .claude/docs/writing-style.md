...
- **Wide display math: break with `aligned` + `={}&` idiom for mobile.** Two-line: `LHS ={}& RHS_part1 \\ &{}+ RHS_part2`. Three-line (when LHS alone is wide): each part on its own row with `&` at the alignment column. The `={}&` / `&{}+` markers keep `=`, `+`, `·` left of the alignment column so continuation lines tuck cleanly under the first. Trust the visual break over `\quad` indents. Apply the same to long inline math when lifting it to display.
- **Closings should echo openings, at every scope.** A section that opens with the attack ("the prover can pick whatever output they like") should close with the resolution ("the verifier now has visibility into the output the prover claims"). A post that opens with "eight curve points, five pairing checks, and a proof that reveals nothing" should land the same triple in its climax. Closes the local arc *and* the post's promise.
- **For recurring incremental constructs, announce upfront then vary subsequent mentions.** If a structure (CRS, ledger, registry) accumulates entries across the post, signal that pattern once after the first introduction. Subsequent additions can then be matter-of-fact ("X is added too" / "Once Y joins") instead of full re-introductions. Variation avoids mechanical "X is also in the CRS" repetition; the upfront signal preserves continuity.
- **Don't pre-empt arguments the section makes later.** If a downstream proof (pairing check + linear independence) forces `r_i = s_i = s_i' = s_i''`, don't also explain "the pieces aren't published separately" earlier as a soft constraint. Trust the algebra to do the work; redundant scaffolding hides the actual mechanism.
- **Bridge algebraic moves with one-line derivations.** "The blinded expression factors:" + dense equation = take-it-on-faith. Adding one bridge ("Expand the products and substitute `LR = HZ + O` for the bare `LR` term — the `+O` cancels with `-O`, leaving everything multiplied by `Z`") walks the reader through the moment of insight. The bridge doesn't replace the equation; it earns it.
- **Pluralize math variables by rewording, not apostrophe-s or bare-s.** `$P_j$'s` reads as possessive (Chicago/AMS discourage); `$P_j$s` collides italic-into-roman. Reword to `$P_j$ values` / `$P_j$ polynomials` / `the $P_j$` to sidestep the contested convention.
- **CRS additions: check whether the element is already published.** Don't say "the CRS exposes X for each public index" if X was already published from an earlier construction (e.g., the KoE setup). Frame it as reuse: "the CRS already has the X they need (from the KoE setup), and the verifier picks the public-index ones." Avoids implying a new CRS addition where there is none.
