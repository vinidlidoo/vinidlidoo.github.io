...
ゼロ知識証明が至るところで話題になっている。Balajiは[こう位置づけている](https://x.com/balajis/status/2022462579713675506)。AIへの対抗手段だ、と。「人工知能は攻撃。ゼロ知識は防御。」[ポッドキャスト](https://zeroknowledge.fm/)、カンファレンス、[暗号のロードマップ](https://strawmap.org/)。ZKはどこにでもある。実際に中で何が起きているのか理解したくて、[予告どおり](@/blog/verkle-trees-polynomial-commitments.ja.md)、数学をゼロから追いかけてみた。最も面白かったのは暗号技術ではなかった。その手前のステップだ。プログラムをどうやって暗号技術が扱える形に変換するのか？

この記事ではそこを扱う。小さなプログラムを取り上げ、基本演算に分解し、それらを行列としてエンコードし、すべてをSNARK[^snark]が検証できる1つの方程式に集約する。構成と例は[Vitalikの2016年のQAP解説](https://medium.com/@VitalikButerin/quadratic-arithmetic-programs-from-zero-to-hero-f6d558cea649)に沿っている。このトピックの入門として、今でも最も明快なものの一つだ。これは全3回の第1回（第2回は証明プロトコル、第3回は応用を扱う）。以前の記事で取り上げた[有限体](@/blog/math-behind-private-key.ja.md)と[多項式コミットメント](@/blog/verkle-trees-polynomial-commitments.ja.md)の知識を前提とする。
...
Aliceは、出力が35になる入力を知っていると主張する。記事の終わりまでに、この主張を1つの多項式の整除性チェックに変換する。各ステップでは実際の解 $x = 3$ を使うが、覚えておいてほしい。本物の証明ではこの値はBobから隠されている。
...
先頭の $1$ は変数ではない。$\text{ゲート } 4$ が三次方程式の「+ 5」をエンコードするときに使う定数項だ。すべての中間値が埋められたこのベクトルが**ウィットネス（witness）**であり、証明者が計算したすべての情報を含んでいる。
...
各ゲート点 $t = i$ で内積 $\mathbf{L}_i \cdot \mathbf{s}$ に評価される単一の多項式が欲しい。便利な性質がこれを可能にする。多項式の重み付き和はそれ自体が多項式だ。重みは係数をスケーリングして組み合わせるだけだ。そこで、ウィットネスのエントリ $s_j$ を列多項式の重みとして使える。
...
有効なウィットネスではすべてのゲート制約が成り立つので、$L(t) \cdot R(t) - O(t) = 0$ が $t = 1, 2, 3, 4$ で成立する。この式を $T(t)$ と呼ぼう。これは $1, 2, 3, 4$ に根を持つ多項式だ。**ターゲット多項式** $Z(t) = (t-1)(t-2)(t-3)(t-4)$ を定義する。同じ4点に根を持つ。
...
**Schwartz-Zippelの補題**が式$(3)$を強力にする。ランダムな $\tau$ で両辺の等式を評価するだけで計算を検証できる。多項式が異なっていれば、偶然通過する確率は無視できるほど小さい。[^sz] 1回のランダムな評価が $n$ 個のゲートチェックすべてを置き換える。本番の回路は数百万のゲートを持つ（Zcash Saplingは約150万、zkEVMロールアップは数千万）。SNARKの「Succinct（簡潔）」はここから来ている。
