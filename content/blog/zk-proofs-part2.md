...
What we have is a working SNARK, modulo a CRS-level patch that closes a remaining soundness gap (ρ, covered in the appendix), and a second scalar γ included in the CRS for soundness-proof reasons.[^gamma] Two gaps remain on the main path:
...
The public half holds the constant 1 and the claimed output. The private half holds the prover's inputs and every intermediate value. From here on, $\pi_A$ sums only over private indices; $\pi_B$ and $\pi_C$ stay as defined, since the verifier needs only one handle on the public values:[^aside]
...
Same argument as before forces the QAP identity, now on $L = L_\text{pub} + L_\text{priv}$: it holds iff $\pi_A, \pi_B, \pi_C, \pi_H$ were computed from a witness matching the verifier's $L_\text{pub}$.[^publicinputs]
...
$$\sum_i (r_i - s_i) L_i(t) + (r_i - s_i') R_i(t) + (r_i - s_i'') O_i(t) \equiv 0$$
...
$$\pi_A = \rho_A L(\tau) \cdot G_1,\quad \pi_B = \rho_B R(\tau) \cdot G_2,\quad \pi_C = \rho_A \rho_B O(\tau) \cdot G_1$$

The β-identity, rewritten with these tags:

$$\sum_i (r_i - s_i) \rho_A L_i(t) + (r_i - s_i') \rho_B R_i(t) + (r_i - s_i'') \rho_A \rho_B O_i(t) \equiv 0$$
...
- Per private variable $i$, A-side: $[\rho_A A_i(\tau)]_1, [\alpha_A \rho_A A_i(\tau)]_1$
- Per private variable $i$, B-side: $[\rho_B B_i(\tau)]_1, [\rho_B B_i(\tau)]_2, [\alpha_B \rho_B B_i(\tau)]_1$
- Per private variable $i$, C-side: $[\rho_A \rho_B C_i(\tau)]_1, [\alpha_C \rho_A \rho_B C_i(\tau)]_1$
- Per private variable $i$, β-bundle: $[\beta(\rho_A A_i(\tau) + \rho_B B_i(\tau) + \rho_A \rho_B C_i(\tau))]_1$
...
$$\pi = \big([A\_{\text{mid}}]_1,\ [A'\_{\text{mid}}]_1,\ [B]_2,\ [B']_1,\ [C]_1,\ [C']_1,\ [K]_1,\ [H]_1\big)$$
...
[^aside]: Splitting all three sides would triple the per-public-input CRS points without adding leverage. The reference appendix has the full post-to-literature notation map.
[^publicinputs]: Some claims also fix *inputs* on the verifier side. The same mechanism carries them: "public" here just means "the verifier fixes the value."
